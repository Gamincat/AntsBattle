...
namespace GamincatKit.Native
{
    public class NativeUtilsPlugin_iOS : NativeUtilsPlugin
    {
        ...
        [DllImport("__Internal")]
        private static extern void _NativeUtils_openReviewDialog(string appId, string title, string message,
            string okButton, string cancelButton, IntPtr actionPtr, _NativeUtils_cs_callback_method callback);
        ...
        public void OpenReviewDialog(string appId, string title, string message, string okButton =
            "OK", string cancelButton = "Cancel", Action<bool> callback = null)
        {
            if (callback == null)
            {
                callback = b =>
            ...
            var callbackIntPtr = (IntPtr)GCHandle.Alloc(callback);
            _NativeUtils_openReviewDialog(appId, title, message, okButton, cancelButton, callbackIntPtr,
                _NativeUtils_cs_callback_method_impl);
        }

        private delegate void _NativeUtils_cs_callback_method(IntPtr gameObjectPtr, bool isOK);
        ...
        {
            var handle = (GCHandle)gameObjectPtr;
            var callback = handle.Target as Action<bool>;
            handle.Free();
